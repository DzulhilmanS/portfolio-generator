 ...
 *   v10/quoteSummary is blocked by Railway datacenter IPs; v7/quote is not.
 */
 ...
 *
 * Why v7 instead of v10/quoteSummary:
 *   Railway datacenter IPs are blocked by Yahoo Finance for v10/quoteSummary.
 *   v7/quote is significantly less blocked and returns enough fundamental fields
 *   for scoring (PE, PB, dividend yield, market cap, beta, 52-week range).
 *   Fields unavailable from v7 (ROE, ROA, margins, analyst breakdown) return null
 *   and the scoring engine handles nulls gracefully.
 *
 * Fallback chain: query1 → query2
 */
export async function fetchFundamentals(ticker) {
  ...
  try {
    ...
    if (!q) {
      console.warn(`Fundamentals failed for ${ticker}: all endpoints blocked`);
    ...
    return {
      ...
      // Fields not available from v7/quote — scoring engine handles nulls
      profitMargin:               null,
